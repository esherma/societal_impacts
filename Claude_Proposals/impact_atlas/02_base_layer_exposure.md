...
For each county, we compute a weighted average of occupation-level AI exposure, where the weights are the share of the local workforce employed in each occupation. Counties with lots of customer service reps and programmers will score high; counties dominated by construction and healthcare will score low.
